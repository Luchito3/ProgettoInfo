...
public class CorsiDialog extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        jLabel3.setText("Durata(numerico)");
        ...
        jLabel5.setText("Gestione Dei Corsi");
    ...
    private void confModifActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_confModifActionPerformed
        ...
        Corso corso = getFormCorso(gg);

        if (corso != null && gg.gestCorsi.cercaCorso(oldMatric.getText()) != null) {
            gg.gestCorsi.cercaCorso(oldMatric.getText()).setAll(corso);

            popolaTabella(gg);
            GestoreFile.scriviCorsi(gg.gestCorsi);
            oldMatric.setText("codice da modificare");
        } else {

            mostraErrore("il corso " + oldMatric.getText() + "non è stato trovato");
            oldMatric.setText("codice da modificare");
        }
    ...
    private void confRemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_confRemActionPerformed
        ...
        if (gg.gestCorsi.cercaCorso(codice) != null) {
            gg.gestCorsi.removeCorso(codice);
            popolaTabella(gg);
            GestoreFile.scriviCorsi(gg.gestCorsi);
            matricRem.setText("codice da rimuovere");
            remPan.setVisible(false);
        } else {
            mostraErrore("Il corso " + codice + " non è stato trovato");
             oldMatric.setText("codice da rimuovere");
    ...
    private void AggiungibtActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_AggiungibtActionPerformed
        ...
        Corso corso=getFormCorso(gg);
        if(corso!=null){
        if(gg.gestCorsi.cercaCorso(corso.getCodice())!=null){
            
            mostraErrore("corso già presente");
        }else{
            
            gg.gestCorsi.addCorso(corso);
             popolaTabella(gg);
            GestoreFile.scriviCorsi(gg.gestCorsi);
        }
        }else{
    ...
    public void popolaTabella(GestoreGenerale gg) {
        ...
        for (Corso d : gg.gestCorsi.getCorsi()) {
            Object[] riga = {
                d.getCodice(),
                d.getNome(),
                d.getDurata(),
...
public Corso getFormCorso(GestoreGenerale gg) {
    String codice = Matricolatx.getText().trim();
    String nome = Nometx.getText().trim();
    String durataStr = Cognometx.getText().trim();

    if (codice.isEmpty() || nome.isEmpty() || durataStr.isEmpty()) {
        mostraErrore("Tutti i campi sono obbligatori ed i codici esigono essere validi");
        return null;
    }
    ...
    try {
        ...
        return new Corso(codice, nome, durata);
    } catch (NumberFormatException e) {
        mostraErrore("La durata deve essere un numero intero valido");
        return null;
    }
}
