...
public class DocentiDialog extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        TabellaDoc.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Matricola", "Nome", "Cognome", "CodDisciplina"
            }
        ) {
        ...
        jLabel5.setText("Gestione Dei Docenti");
    ...
    private void confModifActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_confModifActionPerformed
        ...
        Docente prof = getFormDocente(gg);

        if (prof != null && gg.gestDocenti.cercaDocente(oldMatric.getText()) != null) {
            gg.gestDocenti.cercaDocente(oldMatric.getText()).setAll(prof);

            popolaTabella(gg);
            GestoreFile.scriviDocenti(gg.gestDocenti);
            oldMatric.setText("matricola da modificare");
        } else {

            mostraErrore("il docente " + oldMatric.getText() + "non è stato trovato");
    ...
    private void confRemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_confRemActionPerformed
        ...
        if (gg.gestDocenti.cercaDocente(matricola) != null) {
            gg.gestDocenti.rimuoviDocente(matricola);
            popolaTabella(gg);
            GestoreFile.scriviDocenti(gg.gestDocenti);
            matricRem.setText("matricola da rimuovere");
            remPan.setVisible(false);
        } else {
            mostraErrore("Il docente " + matricola + " non è stato trovato");
    ...
    private void AggiungibtActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_AggiungibtActionPerformed
        ...
        Docente prof=getFormDocente(gg);
        if(prof!=null){
        if(gg.gestDocenti.cercaDocente(prof.getMatricola())!=null){
            
            mostraErrore("docente già presente");
        }else{
            
            gg.gestDocenti.addDocente(prof);
             popolaTabella(gg);
            GestoreFile.scriviDocenti(gg.gestDocenti);
        }
        }else{
    ...
    public void popolaTabella(GestoreGenerale gg) {
        ...
        for (Docente d : gg.gestDocenti.getListaDocenti()) {
            Object[] riga = {
                d.getMatricola(),
                d.getNome(),
                d.getCognome(),
    ...
    public Docente getFormDocente(GestoreGenerale gg) {
        String matricola = Matricolatx.getText().trim();
        String nome = Nometx.getText().trim();
        String cognome = Cognometx.getText().trim();
        String codiceDisciplina = CDisctx.getText().trim();

        if (matricola.isEmpty() || nome.isEmpty() || cognome.isEmpty() || codiceDisciplina.isEmpty()) {
            mostraErrore("TUTTI I CAMPI SONO OBBLIGATORI");
        } else {
            if (gg.gestDiscipline.cercaDisciplina(codiceDisciplina) != null) {
                Docente prof = new Docente(matricola, nome, cognome, gg.gestDiscipline.cercaDisciplina(codiceDisciplina));
                return prof;
            } else {
